# Daemonless Version Status

| Status | Image | Tag | Version |
|:------:|-------|-----|---------|
| :material-check-circle:{ .current } | adguardhome | pkg | 0.107.69_1 |
| :material-check-circle:{ .current } | adguardhome | pkg-latest | 0.107.71 |
| :material-check-circle:{ .current } | adguardhome | latest | 0.107.71 |
| :material-check-circle:{ .current } | adguardhome-sync | latest | 0.8.2 |
| :material-check-circle:{ .current } | cloudflared | pkg | 2025.8.0_4 |
| :material-check-circle:{ .current } | cloudflared | pkg-latest | 2025.8.0_5 |
| :material-check-circle:{ .current } | gitea | pkg | 1.24.2_7 |
| :material-check-circle:{ .current } | gitea | pkg-latest | 1.24.2_8 |
| :material-check-circle:{ .current } | homepage | pkg | 1.8.0 |
| :material-check-circle:{ .current } | homepage | pkg-latest | 1.8.0 |
| :material-check-circle:{ .current } | homepage | latest | 1.9.0 |
| :material-check-circle:{ .current } | hugo | pkg | 0.154.5_1 |
| :material-check-circle:{ .current } | hugo | pkg-latest | 0.154.5_1_1 |
| :material-check-circle:{ .current } | hugo | latest | 0.154.5 |
| :material-close-circle:{ .outdated } | immich-ml | latest | 2.4.1 → **2.5.0** |
| :material-close-circle:{ .outdated } | immich-server | latest | 2.4.1 → **2.5.0** |
| :material-check-circle:{ .current } | jellyfin | pkg | 10.11.5 |
| :material-check-circle:{ .current } | jellyfin | pkg-latest | 10.11.5 |
| :material-check-circle:{ .current } | lidarr | pkg | 2.13.3.4711_1 |
| :material-check-circle:{ .current } | lidarr | pkg-latest | 3.1.0.4875 |
| :material-check-circle:{ .current } | lidarr | latest | 3.1.0.4875 |
| :material-check-circle:{ .current } | mealie | latest | 3.9.2 |
| :material-check-circle:{ .current } | n8n | latest | 2.4.6 |
| :material-check-circle:{ .current } | nextcloud | pkg | 32.0.3_4 |
| :material-check-circle:{ .current } | nextcloud | pkg-latest | 32.0.5 |
| :material-check-circle:{ .current } | openspeedtest | latest | 2.0.5 |
| :material-check-circle:{ .current } | overseerr | latest | 1.34.0 |
| :material-check-circle:{ .current } | plex | latest | 1.43.0.10467-2b1ba6e69 |
| :material-check-circle:{ .current } | postgres-14 | pkg | 14.20 |
| :material-check-circle:{ .current } | postgres-14 | pkg-latest | 14.20 |
| :material-check-circle:{ .current } | postgres-17 | pkg | 17.7 |
| :material-check-circle:{ .current } | postgres-17 | pkg-latest | 17.7 |
| :material-check-circle:{ .current } | prowlarr | pkg | 2.0.5.5160_1 |
| :material-check-circle:{ .current } | prowlarr | pkg-latest | 2.3.0.5236 |
| :material-check-circle:{ .current } | prowlarr | latest | 2.3.0.5236 |
| :material-check-circle:{ .current } | radarr | pkg | 5.28.0.10274 |
| :material-check-circle:{ .current } | radarr | pkg-latest | 6.0.4.10291 |
| :material-check-circle:{ .current } | radarr | latest | 6.0.4.10291 |
| :material-check-circle:{ .current } | redis | pkg | 8.4.0_1 |
| :material-check-circle:{ .current } | redis | pkg-latest | 8.4.0_1 |
| :material-check-circle:{ .current } | sabnzbd | pkg | 4.5.5 |
| :material-check-circle:{ .current } | sabnzbd | pkg-latest | 4.5.5 |
| :material-check-circle:{ .current } | sabnzbd | latest | 4.5.5 |
| :material-check-circle:{ .current } | smokeping | pkg | 2.9.0_1 |
| :material-check-circle:{ .current } | smokeping | pkg-latest | 2.9.0_1 |
| :material-check-circle:{ .current } | smokeping | latest | 2.9.0_1 |
| :material-check-circle:{ .current } | sonarr | pkg | 4.0.16.2944 |
| :material-check-circle:{ .current } | sonarr | pkg-latest | 4.0.16.2944 |
| :material-check-circle:{ .current } | sonarr | latest | 4.0.16.2944 |
| :material-check-circle:{ .current } | tailscale | pkg | 1.92.5 |
| :material-check-circle:{ .current } | tailscale | pkg-latest | 1.92.5_1 |
| :material-check-circle:{ .current } | tautulli | pkg | 2.16.0_1 |
| :material-check-circle:{ .current } | tautulli | pkg-latest | 2.16.0_1 |
| :material-check-circle:{ .current } | tautulli | latest | 2.16.0 |
| :material-check-circle:{ .current } | traefik | pkg | 3.6.6 |
| :material-check-circle:{ .current } | traefik | pkg-latest | 3.6.6_1 |
| :material-check-circle:{ .current } | traefik | latest | 3.6.7 |
| :material-check-circle:{ .current } | transmission | pkg | 4.0.6_7 |
| :material-check-circle:{ .current } | transmission | pkg-latest | 4.0.6_7 |
| :material-check-circle:{ .current } | transmission-wireguard | pkg | 4.0.6_7 |
| :material-check-circle:{ .current } | transmission-wireguard | pkg-latest | 4.0.6_7 |
| :material-check-circle:{ .current } | unifi | pkg | 10.0.160 |
| :material-check-circle:{ .current } | unifi | pkg-latest | 10.0.160 |
| :material-check-circle:{ .current } | unifi | latest | 10.0.162 |
| :material-check-circle:{ .current } | uptime-kuma | latest | 2.0.2 |
| :material-check-circle:{ .current } | vaultwarden | pkg | 1.35.2 |
| :material-check-circle:{ .current } | vaultwarden | pkg-latest | 1.35.2 |
| :material-close-circle:{ .outdated } | woodpecker | latest | 3.12.0 → **3.13.0** |

## Summary

- :material-check-circle:{ .current } Current: 30
- :material-close-circle:{ .outdated } Outdated: 3
